...
class OrdererInfoRoute extends Component {
  ...
  handleDelete = async () => {
    ...
    OrdererActions.fetchingOrdererData({
      fetch: true,
      message: <Loader>{name} 님의 정보를 삭제중...</Loader>,
    });

    await api
      .deleteOrderer({ id: userid })
      .then((res) => {
        OrdererActions.fetchingOrdererData({
          fetch: true,
          message: (
        ...
        setTimeout(() => {
          const restList = orderer
            .get("data")
            .filter((d) => d.get("_id") !== userid);
  ...
  render() {
    ...
    const { index, data, view_past, del_open, hide, random } = this.state;
    const {
      handleModal,
      handleDelete,
      handleCancel,
    ...
    return (
      <OrdererInfo>
        <InfoCard
          backgroundImage={backImages[random]}
          ...
          onViewPastClick={handleViewPastClick}
        >
          <InfoList
            list={{
              ordererInfo,
              bViewPast: view_past,
              data: !utils.empty(data)
                ? { total: data["total"], graph: data["graph"] }
                : null,
            }}
          />
            ...
            <Modal.Content>
              <Message error icon>
                ...
                <Message.Content>
                  ...
                  >
                    <b>'{ordererInfo.name}'</b> 님의 정보를 삭제하시겠습니까?
                  </Message.Header>
                  <p style={{ color: "red" }}>
                    삭제를 하실 경우에는 신중히 고려하신 후에 결정하십시요!!!
                  </p>
                </Message.Content>
              </Message>
            </Modal.Content>

...
import DayPickerCaption from "components/DayPickerCaption";
...
class SearchRoute extends Component {
    ...
    getMonthNotes = async month => {
        this.setState({
            selectedMonth: `${month.getFullYear()}-${month.getMonth()}`
        });
    ...
    handlePickerSearch = async (day, { selected }) => {
        if (!selected) {
            let date = day.toLocaleDateString().replace(/ /g, "");
            if (date.indexOf("/") > -1) {
                date = date.split("/");
                date = [].concat([date[2], date[0], date[1]]);
                date = date.join("-");
            } else {
                date = date.replace(/\./g, "-").substring(0, date.length - 1);
            }
        ...
        return (
            <div className="subcontents-wrapper flex">
                <div className="search-list-wrapper">
                    ...
                    <SearchInput
                        placeholder="찾고 싶은 장부의 내용들을 검색해보세요!"
                        icon
                    ...
                    {searchTxt && (
                        <div className="search-txt">
                            <b>▶</b> 입력하신 검색어는 <span>{searchTxt}</span>
                        </div>
                    )}

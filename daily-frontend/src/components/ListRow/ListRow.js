...
const ListRow = ({ index, data, ordererView, children }) => {
  if (!data) {
    return (
  ...
  Object.keys(delivery).forEach((d, i) => {
    if (d === "date") {
      // console.log(i);
      let dates = moment(new Date(delivery[d])).format(
      ...
      deliveries.push(
        <ListColumn
          key={i}
          text={(delivery[d] * delivery["count"]).toLocaleString()}
          right
        />
      );
    } else if (d === "category") {
      deliveries.push(
        <ListColumn key={i} text={delivery[d]} center className="category" />
      );
    } else {
      if (d !== "count" && d !== "discount") {
        // console.log("Else", i);
